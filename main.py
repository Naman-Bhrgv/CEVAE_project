...
if __name__ == "__main__":
    ...
    parser.add_argument("--beta",type=int,default=1e-3)
    ...
    if args.dataset == "ihdp":
        ...
        print("Synthetic Data running.")
        print("Size- ",args.nsample)
    ...
    for i, (train_loader, test_loader) in enumerate(data_loaders):
        ...
        for epoch in range(args.epochs):
            ...
            if epoch % 5 == 0:
                ...
                print(f"[epoch {epoch:03d}] #TRAIN# ELBO Loss: {total_epoch_loss_train}")
                print(f"[epoch {epoch:03d}] #VALIDATION# ELBO Loss: {total_epoch_loss_test}")

                if not ATT_ONLY:
                    (ITE, ATE, PEHE), (RMSE_factual, RMSE_counterfactual) = \
                        inference.train_statistics(L=1, y_error=True)
                    print(f"[epoch {epoch:03d}] #TRAIN# ITE: {ITE:0.3f}, "
                    ...
                    print(f"[epoch {epoch:03d}] #VALIDATION# ITE: {ITE_test:0.3f}, "
        ...
        if not ATT_ONLY:
            ...
            print(f"[replication {i+1}/{args.replications}] #TRAIN# ITE: {score[0]:0.3f}, "
            ...
            print(f"[replication {i+1}/{args.replications}] #TEST# ITE: {score_test[0]:0.3f}, "
            ...
            print("H1")
            print(f"#TRAIN# ATT: {ATT:0.3f}, Policy Risk: {risk_policy:0.3f}")
            (ATT_test), (risk_policy_test) = inference.test_statistics(L=10, y_error=True)
            print("H2")
            print(f"#TEST# ATT: {ATT_test:0.3f}, Policy Risk: {risk_policy_test:0.3f}")
    ...
    if not ATT_ONLY:
        ...
        print(f"#TRAIN# ATE Error: {torch.mean(train_ate_err):0.3f}+-{torch.std(train_ate_err)}\
        SQRT PEHE: {torch.mean(train_sqrt_pehe):0.3f}+-{torch.std(train_sqrt_pehe)}")
        
        print(f"#TEST# ATE Error: {torch.mean(test_ate_err):0.3f}+-{torch.std(test_ate_err)}\
        ...
        print(f"#TRAIN# ATT Error: {torch.mean(train_att_err):0.3f}+-{torch.std(train_att_err)}\
        Policy Risk: {torch.mean(train_risk_policy):0.3f}+-{torch.std(train_risk_policy)}")
        
        print(f"#TEST# ATT Error: {torch.mean(test_att_err):0.3f}+-{torch.std(test_att_err)}\
