...
class Encoder(nn.Module):
    def __init__(self, binfeats, contfeats, d, h, nh, activation, cuda, multivariate=False):
        ...
        self.muq_t0_sigmaq_t0 = FCNet(
            h, [h], [[d, None], [d * d if multivariate else d, F.softplus]], activation)
    ...
    def forward(self, x, prefix=""):
        ...
        t_name = f"{prefix}t_{id(self)}"

        t = pyro.sample(t_name, dist.Bernoulli(t_logits).to_event(1))
        ...
        y_name = f"{prefix}y_{id(self)}"
        
        y = pyro.sample(y_name, dist.Normal(y_loc, y_scale).to_event(1))
        ...
        # weird numerical instability issue where scale can reach 0?
        z_scale_t0 += 1e-8
        z_scale_t1 += 1e-8
